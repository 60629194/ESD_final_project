...
void processFinalWordAndOutput(unsigned long sendStartUs) {
  ...
  // === 壓縮 784 bits 成 98 bytes，並附加 4 bytes 處理耗時 (總共 102 bytes) ===
  ...
  uint32_t t_c3 = micros() - sendStartUs;
  memcpy(&packet[98], &t_c3, 4);

  Serial.println("無線傳輸資料中...");
  Serial.printf("[Latency] ESP32-C3 構圖處理耗時: %u us\n", t_c3);
...
void checkSerialCommands() {
  ...
  if (cmd.length()>=2) {
    ...
    if (pointCount>0) processFinalWordAndOutput(micros());
  }
}
  ...
  switch (currentState) {
    ...
    case RECORDING_AIR:
      ...
      if (isSendTriggered && !isDrawPressed) {
        ...
        Serial.println("📤 處理中...");
        processFinalWordAndOutput(sendStartUs);
        currentState=IDLE;
        break;
      }
