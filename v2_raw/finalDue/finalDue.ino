...
InfResult run_inference(bool printDetails = true) {
...
const int FRAME_SIZE = msglen + ECC_LEN; // 116
    ...
    if (isTimingBurst) {
      ...
      if (frames_received > 0) {
        ...
        if (repairStatus >= 0) {
            ...
            Serial.println("\n[原始圖片推論]");
            InfResult res = run_inference(true);
            ...
            if (res.score < 12.0 && frames_received > 1) {
                ...
                for (int i = 1; i < min(5, frames_received); i++) {
                    ...
                    if (status >= 0) {
                        populate_input_image(repaired_msg);
                        InfResult adj_res = run_inference(false); 
