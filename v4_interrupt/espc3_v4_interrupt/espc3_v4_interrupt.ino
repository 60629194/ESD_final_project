  ...
  switch (currentState) {
    ...
    case IDLE:
      if (isDrawPressed && !isSendPressed) {
        ...
        neopixelWrite(RGB_BUILTIN, 0, 128, 128); // 寫字時亮青色
      }
      break;
    ...
    case RECORDING_AIR:
      if (currentState==RECORDING_INK && !isDrawPressed) {
        ...
        neopixelWrite(RGB_BUILTIN, 0, 0, 0); // 抬筆熄滅
      }
      else if (currentState==RECORDING_AIR && isDrawPressed && !isSendPressed) {
        currentState=RECORDING_INK;
        currentStrokes++;
        neopixelWrite(RGB_BUILTIN, 0, 128, 128); // 再次落筆亮青色
      }
      ...
      if (isSendTriggered && !isDrawPressed) {
        ...
        delay(100); // 讓紅燈維持一下
        neopixelWrite(RGB_BUILTIN, 0, 0, 0); // 關閉
